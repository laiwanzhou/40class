...
from src.data.clean_skeleton_dataset import CleanSkeletonSegmentDataset
from src.engine import run_epoch
from src.models import SegmentAwareTemporalClassifier
from src.train_skeleton_c0_c1_strict_oof import (
    formal_refit_and_predict, resolve, set_normalization, set_seed, sha256, summarize, train_selection,
)


PROJECT_ROOT = Path(__file__).resolve().parents[1]
MODEL_NAME = "T1_segment_aware_c1_tcn"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run frozen T1-v1 Segment-Aware C1-TCN strict OOF experiment.")
    parser.add_argument(
        "--config", type=Path,
        default=PROJECT_ROOT / "configs/experiments/skeleton_segment_aware_tcn_strict_oof.yaml",
    )
...
def model_for(config: dict[str, Any]) -> SegmentAwareTemporalClassifier:
    return SegmentAwareTemporalClassifier(
        input_features=int(config["input_features"]), embedding_dim=int(config["embedding_dim"]),
        num_classes=int(config["num_classes"]),
        channels=tuple(int(value) for value in config["tcn_channels"]), dropout=float(config["dropout"]),
    )
...
def dataset_for(
    ...
    if str(config["scale_policy"]) != "per_frame" or int(config["input_features"]) != 102:
        raise ValueError("T1-v1 is frozen to the 102D C1 representation")
    return CleanSkeletonSegmentDataset(
        view, data_root, set(users), "per_frame", int(config["sequence_length"])
    )
...
def run_smoke_test(config: dict[str, Any], assignment: dict[str, Any], device: torch.device) -> None:
    ...
    metrics = run_epoch(
        model, batch_loader, nn.CrossEntropyLoss(), device, bool(config["amp"]) and device.type == "cuda",
        optimizer=optimizer, scaler=scaler, max_batches=1, gradient_clip=float(config["gradient_clip"]),
    )
    print(json.dumps({
...
def run_fold(config: dict[str, Any], fold: dict[str, Any], device: torch.device) -> dict[str, Any]:
    ...
    predictions, metrics, by_user = formal_refit_and_predict(
        config, outer_train, outer_validation, selected_epoch, device, seed, run_dir, model_builder=model_for
    )
    ...
    summary = {
        ...
        "temporal_edge_contract": "valid_source_and_destination_with_equal_retained_segment_id",
