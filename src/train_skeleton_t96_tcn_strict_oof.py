...
MODEL_NAME = "T2_c1_tcn_96"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run frozen T2-v1 96-step C1-TCN strict OOF experiment.")
    parser.add_argument(
        "--config", type=Path,
        default=PROJECT_ROOT / "configs/experiments/skeleton_t96_tcn_strict_oof.yaml",
    )
...
def dataset_for(
    ...
    if str(config["scale_policy"]) != "per_frame" or int(config["input_features"]) != 102:
        raise ValueError("T2-v1 is frozen to the joint-only C1 representation")
    if int(config["sequence_length"]) != 96:
        raise ValueError("T2-v1 is frozen to 96 temporal steps")
    return CleanSkeletonDataset(view, data_root, set(users), "per_frame", 96)
...
def run_smoke_test(config: dict[str, Any], assignment: dict[str, Any], device: torch.device) -> None:
    ...
    print(json.dumps({
        ...
        "sequence_length": 96, "parameter_count": sum(parameter.numel() for parameter in model.parameters()),
...
def run_fold(config: dict[str, Any], fold: dict[str, Any], device: torch.device) -> dict[str, Any]:
    ...
    summary = {
        "representation": MODEL_NAME, "scale_policy": "per_frame", "sequence_length": 96,
        "model_class": "TemporalClassifier", "fold": fold_index, "selected_epoch": selected_epoch,
