...
from src.data.clean_skeleton_dataset import CleanSkeletonGraphDataset
from src.engine import run_epoch
from src.models.lightweight_stgcn import LightweightSTGCN
...
MODEL_NAME = "lightweight_stgcn_c1"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run lightweight ST-GCN on strict Skeleton C1 OOF views.")
    parser.add_argument(
        "--config", type=Path,
        default=PROJECT_ROOT / "configs/experiments/skeleton_lightweight_stgcn_strict_oof.yaml",
    )
...
def model_for(config: dict[str, Any]) -> LightweightSTGCN:
    return LightweightSTGCN(
        input_channels=int(config["input_channels"]),
        channels=tuple(int(value) for value in config["stgcn_channels"]),
...
def dataset_for(view: Path, data_root: Path, users: list[str], config: dict[str, Any]) -> CleanSkeletonGraphDataset:
    if str(config["scale_policy"]) != "per_frame":
        raise ValueError("Lightweight ST-GCN v1 is frozen to C1 per-frame scale")
    return CleanSkeletonGraphDataset(
...
def run_smoke_test(config: dict[str, Any], assignment: dict[str, Any], device: torch.device) -> None:
    ...
    dataset = dataset_for(view, resolve(config["data_root"]), list(fold["epoch_selection"]["fit_user_ids"]), config)
    subset = Subset(dataset, range(min(2, len(dataset))))
    batch_size = len(subset)
    batch_loader = DataLoader(subset, batch_size=batch_size, shuffle=False, num_workers=0)
    ...
    print(json.dumps({
        ...
        "scope": "inner_selection_fit_users_only", "samples": batch_size,
        "parameter_count": parameter_count, "loss": metrics["loss"], "device": str(device),
    }, indent=2))
...
def run_fold(config: dict[str, Any], fold: dict[str, Any], device: torch.device) -> dict[str, Any]:
    ...
    summary = {
        ...
        "segment_aware_temporal_convolution": True,
    }
