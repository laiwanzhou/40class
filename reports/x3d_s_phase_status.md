...
| Phase 4: Train-14 OOF scientific evaluation | Completed under compute amendment | `ac5fc7e` + closure pending | Three X3D seeds, nine-checkpoint exact regeneration, canonical diagnostics, and fixed fold-0 x 10-epoch matched sanity are complete; X3D is competition-retained | Seed17 fold1 used the approved 28/30-epoch recovery; original full matched three-fold primary rule and paired CI were not evaluated; `>64` bucket remains weak | Commit closure, then begin Phase 5 evidence registration |
| Phase 5: Register IR sparse evidence | Pending | - | Pure candidate registers as `ir_x3d_s_k400_pure` using canonical seed `20260715` only | Held-out archive is evaluation-only until Phase 10 | Begin after Phase 4 closure commit |
