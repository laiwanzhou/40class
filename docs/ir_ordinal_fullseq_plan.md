...
- [x] **Stage 8 complete: fixed three-representation pilot and selection.** `relative` was selected: Accuracy/Macro-F1 `0.2746/0.1207`, compared with `raw` `0.2525/0.0975` and `raw+relative` `0.2627/0.1017` at their best-Macro checkpoints.
- [x] **Stage 9 complete: selected-relative formal experiment safely stopped after Epoch 16.** Best Accuracy was `0.2780` at Epoch 11; best Macro-F1 was `0.1585` at Epoch 14. Train Accuracy reached `0.5940` while stopped-point validation Accuracy fell to `0.2186`, confirming severe train-user overfitting and a large regression from B2. The user-requested sentinel stopped at an epoch boundary after history/checkpoint writes and best-checkpoint reevaluation.
