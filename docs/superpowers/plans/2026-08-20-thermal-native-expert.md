...
**T1-B.3 zero-training block attribution (2026-08-20):** The frozen T1-B.1 threshold exactly reconstructed all 23 validation spikes: 12 class36 and 11 across classes 4/6/7/8/11/14/24/28. Every spike received a same-user/same-class stable control. Official ImageNet pretrained iFormer-T showed no block4/5 spike-control separation (FP32 block5 project ratio 1.005x), while epoch16 showed 2.518x overall and 2.723x for class36; skip input was only 1.289x and add output 1.790x. Every 10x event arose in the project/residual convolution branch before add, with no residual-alignment event; `gamma` is absent. Block conv-weight drift stayed below 1.99% and median BN eval-gain ratios below 1.084x, consistent with compounding small changes rather than one gross block-local gain change. At epoch16 FP32 block5, 17/23 spikes were distributed across the sequence, 4 used a few segments, and 2 were single-segment; the three largest spikes were respectively distributed, few-segment, and few-segment (bfloat16: distributed, few, single). Class36 is enriched and stronger but not exclusive. This supports finetuning-induced convolution-branch amplification with an extreme tail, not a pretrained Thermal/architecture response or residual-add alignment. All states, BN buffers, logits, embeddings, and checkpoint SHA remained unchanged. Stop for human decision; no new training experiment is authorized.
