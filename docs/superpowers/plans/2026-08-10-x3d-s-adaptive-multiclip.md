...
- [x] **Step 2: Run the fixed three-fold user-grouped OOF experiment**
...
- [x] **Step 3: Re-evaluate the selected fold checkpoints**
...
**Competition-compute amendment (2026-08-12):** The complete matched MobileNet/TCN run was stopped after fold-0 inner epochs 1-4, before any formal refit or outer-validation prediction, because its expected 3-4 GPU-hour cost has low decision value after three X3D seeds and the large gap to historical visual baselines. This amendment is transparently post-start: the four inner-validation rows had been observed, so the abandoned run is preserved and cannot be represented as a completed preregistered comparison. Replace it with one fresh fixed-budget sanity check: canonical seed `20260715`, frozen outer fold 0, all fold-0 outer-train users, exactly 10 epochs using the unchanged 30-epoch cosine prefix, no inner selection, and exactly one outer-validation evaluation after the checkpoint is frozen. If fold-0 Accuracy is below `0.53`, stop baseline compute and retain pure X3D for the competition program based on three-seed X3D stability plus historical evidence; do not claim the original matched three-fold paired-bootstrap primary rule was completed. If Accuracy is at least `0.53`, the baseline is unexpectedly close and the full matched three-fold experiment plus paired bootstrap must be reconsidered before Phase 5. This amendment optimizes competition gain per GPU-hour and does not change the canonical X3D evidence.

- [x] **Step 5: Apply the amended competition-retention rule**
...
- **Amended competition retention:** When the fixed 10-epoch fold-0 sanity baseline is below `0.53` Accuracy, retain pure X3D as the IR expert for Phase 5 under the compute-budget amendment. Label the decision `competition-retained; full matched primary rule not evaluated`, preserve the sanity checkpoint/predictions, and report the fold-0 gap without a three-fold confidence interval.
